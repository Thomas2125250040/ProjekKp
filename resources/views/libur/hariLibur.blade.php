                    ...
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="{{ route('hari-libur.destroy', $row->id) }}">
                                @csrf
                                @method('delete')
                                <button type="submit" class="btn btn-danger fs-1 hapus_hari-libur" data-toggle="tooltip"
                                    title='Delete' data-nama='{{ $row->keterangan }}'>Hapus</button>
                                <a href="{{ route('hari-libur.edit', $row->id) }}"
